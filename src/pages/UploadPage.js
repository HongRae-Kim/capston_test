...
function UploadPage() {
    ...
    useEffect(() => {
        // sessionStorage에 저장된 파일과 결과 데이터가 있으면 가져옴
    ...
    useEffect(() => {
        // 파일과 결과 데이터가 변경될 때마다 sessionStorage에 저장
        if (file) {
                ...
                <div className="column-wrapper">
                    ...
                    {resultData && (
                        <div className="result-section margin-medium">
                            <h3>예측 결과</h3>
                            <p>혈관 나이 예측 결과: <strong>{resultData.predicted_class}</strong></p>
                            <p>혈관 나이: <strong>{resultData.vascular_age}</strong></p>
                            <p>혈관 점수: <strong>{resultData.vascular_score}</strong></p>
                            <p>노화 속도: <strong>{resultData.aging_speed}</strong></p>
                            <p>예측 확률: <strong>{(resultData.predicted_probability * 100).toFixed(2)}%</strong></p>
                            <button
